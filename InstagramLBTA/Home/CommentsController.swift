...
class CommentsController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        dummyCell.layoutIfNeeded() // layouts subviews immediately, use to force layout subviews before drawing
    ...
    lazy var containerView: UIView = {
        ...
        lineSeperatorView.anchor(top: containerView.safeAreaLayoutGuide.topAnchor, left: containerView.leftAnchor, bottom: nil, right: containerView.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0.5)
    ...
    @objc func handleSubmit() {
        ...
        Database.database().reference().child("comments").child(postID).childByAutoId().updateChildValues(values) { (err, ref) in
            if let err = err {
                print("Failed to insert commment", err)
                return
            }
